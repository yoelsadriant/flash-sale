...
describe('auth helpers', () => {
  ...
  test('getUser returns null when no session stored', async () => {
    const { getUser } = await import('../../src/api/auth');
    expect(getUser()).toBeNull();
  });

  test('getStoredToken returns null when no token stored', async () => {
    const { getStoredToken } = await import('../../src/api/auth');
    expect(getStoredToken()).toBeNull();
  });

  test('storeSession persists token and user', async () => {
    const { storeSession, getStoredToken, getUser } = await import('../../src/api/auth');
  ...
  test('logout clears stored session', async () => {
    const { storeSession, logout, getStoredToken, getUser } = await import('../../src/api/auth');
    storeSession(MOCK_TOKEN, MOCK_USER);
    logout();
    expect(getStoredToken()).toBeNull();
    expect(getUser()).toBeNull();
  ...
  test('getAuthHeaders returns empty object when no token', async () => {
    const { getAuthHeaders } = await import('../../src/api/auth');
    expect(getAuthHeaders()).toEqual({});
  });

  test('getAuthHeaders returns Bearer token when token is stored', async () => {
    const { storeSession, getAuthHeaders } = await import('../../src/api/auth');
    storeSession(MOCK_TOKEN, MOCK_USER);
    expect(getAuthHeaders()).toEqual({ Authorization: `Bearer ${MOCK_TOKEN}` });
  });

  test('getUser returns null when stored value is invalid JSON', async () => {
    localStorage.setItem('flash-sale.user', 'not-valid-json{{{');
    const { getUser } = await import('../../src/api/auth');
    expect(getUser()).toBeNull();
  });
});
